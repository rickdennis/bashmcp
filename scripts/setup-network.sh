...
if [ -n "${FC_EGRESS_ENABLED:-}" ] && [ "${FC_EGRESS_ENABLED}" != "0" ]; then
    ...
    echo "==> Enabling fc-egress TPROXY interception of 172.16.0.0/24:443 -> ${EGRESS_ADDR}"
...
table inet fcegress {
  chain prerouting {
    ...
    ip saddr 172.16.0.0/24 tcp dport 443 meta mark set 0x1 tproxy ip to ${EGRESS_IP}:${EGRESS_PORT}
  }
}
NFT
    echo "    nft table inet fcegress installed (fwmark 0x1 -> table 100 -> lo)"
fi

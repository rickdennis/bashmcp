...
FC_AGENT_BIN="${FC_AGENT_BIN:-$REPO_DIR/bin/fc-agent-$_fc_arch}"   # built by build-agent.sh
...
# with: bash scripts/build-agent.sh (produces the per-arch binaries in bin/).
...
if [[ "${FC_BAKE_AGENT_RUNTIME:-1}" != "0" ]]; then
    ...
    cp "$REPO_DIR/agent-runner/runner.mjs"      "$BUILD/runner.mjs"
    cp "$REPO_DIR/agent-runner/fc-agent-runner"  "$BUILD/fc-agent-runner"
